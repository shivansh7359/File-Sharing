...
import { Box, Button, Checkbox, CircularProgress, Grid, Grid2, TextField, Typography } from '@mui/material'
...
const Login = () => {
    ...
    const handleSubmit = async() => {
        try{
            setLoading(true)
            const result = await axios.post(`${process.env.REACT_APP_API_URL}/auth/login`, {email, password})
    ...
    >
        <Grid2 container spacing={2} sx={{width: '60%', border: '1px solid black', padding: '20px', borderRadius: '8px',}}>
            <Grid2 size={{xs: 12, md: 12}}>
                <Typography sx={{fontWeight: 'bold', textAlign: 'center'}}>Login</Typography>
            </Grid2> 
